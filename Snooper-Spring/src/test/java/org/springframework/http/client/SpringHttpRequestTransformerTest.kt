...
class SpringHttpRequestTransformerTest {
    ...
    fun shouldTransformHttpCallFromSpringHttpRequest() {
        ...
        assertThat(
            httpCall.requestHeaders.size,
            `is`(1)
        )

        assertThat(
            httpCall.responseHeaders.size,
            `is`(1)
        )

        assertNotNull(
            httpCall.responseHeaders[CONTENT_TYPE]
        )

        assertNotNull(
            httpCall.requestHeaders[CONTENT_TYPE]
        )
    }
    ...
    fun shouldTransformHttpCallFromClientSideError() {
        ...
        assertThat(
            httpCall.error,
            `is`(
                "java.net.UnknownHostException: Unable to connect"
            )
        )
    }
